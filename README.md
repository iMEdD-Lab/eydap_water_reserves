...
## Files
The repository contains 3 .csv, one .py and one .yml files: 
...
The eydap_rss_downloader.py contains the code that is run daily (except weekends, when EYDAP does not refreshes its data) via Github Actions (with the file update_water_reserves.yml). It gets the RSS feed from EYDAP, formats the data as a dataframe, loads the historical data and builds a dataframe that contains both the historical and the latest recordings. 
